Imports System.Text
Public Class hrU14_TinhTrangSucKhoe
    ...
    Protected Sub btnAddRecord_Click(sender As Object, e As EventArgs) Handles btnAddRecord.Click
        ...
        Dim NgayKham As String = txtNgayKham.Text
        Dim NoiDungKham As String = txtNoiDungKham.Text
        Dim NoiKham As String = txtNoiKham.Text
        Dim TinhTrangSucKhoe As String = txtTinhTrangSucKhoe.Text
        Dim MaNhanVien As String = Session("MaNhanVien")

        Dim parameters = SqlDataSource1.InsertParameters

        parameters("NgayKham").DefaultValue = NgayKham
        parameters("NoiDungKham").DefaultValue = NoiDungKham
        parameters("NoiKham").DefaultValue = NoiKham
        parameters("TinhTrangSucKhoe").DefaultValue = TinhTrangSucKhoe
        parameters("MaNhanVien").DefaultValue = MaNhanVien

        Try
            If (txtNgayKham.Text = "") Then
    ...
    Protected Sub GridView1_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView1.RowCommand
        ...
        If (e.CommandName.Equals("editRecord")) Then
            ...
            txtMaSucKhoe1.Text = HttpUtility.HtmlDecode(gvrow.Cells(2).Text).ToString()
            txtNgayKham1.Text = HttpUtility.HtmlDecode(gvrow.Cells(3).Text).ToString().Trim()
            txtNoiDungKham1.Text = HttpUtility.HtmlDecode(gvrow.Cells(4).Text).ToString().Trim()
            txtNoiKham1.Text = HttpUtility.HtmlDecode(gvrow.Cells(5).Text).ToString().Trim()
            txtTinhTrangSucKhoe1.Text = HttpUtility.HtmlDecode(gvrow.Cells(6).Text).ToString().Trim()
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Dim MaSucKhoe As String = txtMaSucKhoe1.Text
        Dim NgayKham As String = txtNgayKham1.Text
        Dim NoiDungKham As String = txtNoiDungKham1.Text
        Dim NoiKham As String = txtNoiKham1.Text
        Dim TinhTrangSucKhoe As String = txtTinhTrangSucKhoe1.Text
        Dim MaNhanVien As String = Session("MaNhanVien")

        Dim parameters = SqlDataSource1.UpdateParameters

        parameters("MaSucKhoe").DefaultValue = MaSucKhoe
        parameters("NgayKham").DefaultValue = NgayKham
        parameters("NoiDungKham").DefaultValue = NoiDungKham
        parameters("NoiKham").DefaultValue = NoiKham
        parameters("TinhTrangSucKhoe").DefaultValue = TinhTrangSucKhoe
    ...
    Private Sub executeDelete(ByVal code As String)
        If IsValid Then
            ...
            parameters("MaSucKhoe").DefaultValue = code
